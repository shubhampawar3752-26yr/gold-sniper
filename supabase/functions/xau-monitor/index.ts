...
function hit(p, t, d) { return d === 'long' ? p >= t : p <= t; }
function slHit(p, s, d) { return d === 'long' ? p <= s : p >= s; }

async function fetchC(sym, interval, range) {
...
function chkTick(px, s, l, prev, al) {
  ...
  if (!s.tp1Hit && hit(px, s.tp1, s.dir)) {
    s.tp1Hit = true;
    if (!prev.tp1) al.push({ type: 'tp', timeframe: l, tp_num: 1, tp_price: s.tp1, price: px, progress: 1, sent: false });
  }
  if (s.tp1Hit && !s.tp2Hit && hit(px, s.tp2, s.dir)) {
    s.tp2Hit = true;
    if (!prev.tp2) al.push({ type: 'tp', timeframe: l, tp_num: 2, tp_price: s.tp2, price: px, progress: 2, sent: false });
  }
  if (s.tp2Hit && !s.tp3Hit && hit(px, s.tp3, s.dir)) {
    s.tp3Hit = true;
    if (!prev.tp3) al.push({ type: 'tp', timeframe: l, tp_num: 3, tp_price: s.tp3, price: px, progress: 3, sent: false });
  }
  if (s.tp3Hit && !s.tp4Hit && hit(px, s.tp4, s.dir)) {
    s.tp4Hit = true;
    if (!prev.tp4) al.push({ type: 'tp', timeframe: l, tp_num: 4, tp_price: s.tp4, price: px, progress: 4, sent: false });
  }
  if (s.tp4Hit && !s.tp5Hit && hit(px, s.tp5, s.dir)) {
    s.tp5Hit = true;
    if (!prev.tp5) al.push({ type: 'tp', timeframe: l, tp_num: 5, tp_price: s.tp5, price: px, progress: 5, sent: false });
  }
  if (s.tp1Hit && s.tp2Hit && s.tp3Hit && s.tp4Hit && s.tp5Hit) {
    if (!prev.allDone) {
      s.allDone = true;
      al.push({ type: 'alldone', timeframe: l, entry: s.entry, cycle: s.cycle, price: px, sent: false });
    }
  }
}
...
function ns() {
  return {
    ...
    allDone: false, cycle: 1, lastSignal: null,
  };
}

function setLevels(s, a) {
...
Deno.serve(async (req) => {
  ...
  for (const tf of TFS) {
    ...
    try {
      ...
      if (cr && done && s.lastSignal !== cr.signal) {
        ...
        setLevels(s, a);
        alerts.push({
          type: 'entry', timeframe: l, direction: cr.signal, entry: s.entry, sl: s.sl,
          tp: { tp1: s.tp1, tp2: s.tp2, tp3: s.tp3, tp4: s.tp4, tp5: s.tp5 }, cycle: s.cycle, price: px, sent: false,
        });
      }
      if (s.entry === 0) {
        const ac = allCross(c);
    ...
    } catch (e) { console.error(`TF ${l}:`, e.message); }
  }
  ...
  for (const a of alerts) {
    try { await supabase.from('alerts').insert(a); } catch (e) { console.error('Alert:', e.message); }
  }

  try {
    if (sid) {
      await supabase.from('trading_states').update({ states, prev_hits: prev, last_run: now }).eq('id', sid);
    } else {
      const { data: cr } = await supabase.from('trading_states').insert({ states, prev_hits: prev, last_run: now }).select().single();
      sid = cr?.id || null;
    }
  ...
  return new Response(JSON.stringify({
    ...
    timestamp: now,
    mode: 'tick-by-tick',
    ticksCaptured: tl.length,
    lastTickPrice: lt?.price || null,
    ticks: tl,
    stateId: sid,
    timeframesChecked: TFS.length,
