...
Deno.serve(async (req) => {
  ...
  for (const tf of TFS) {
    ...
    // Group by cycle and show: Entry, SL, TP1-TP5 hit status with prices
    ...
    for (const a of d.tps) {
      let c = a.cycle;
      if (!c) {
        const matched = findEntryByTime(tf, a.created_at);
        if (matched) c = matched.cycle;
      }
      const cy = getCycle(c);
      ...
      cy.tps.set(tpN, { price: Number(a.tp_price || a.tpPrice || 0), hit: true });
    }
    ...
    if (cycles.size > 0) {
      ...
      const sortedCycles = Array.from(cycles.entries()).sort(function(a, b) { return b[0] - a[0]; });
      for (const pair of sortedCycles) {
        ...
        // Get TP prices from trading state or entry alert
        const s = state[tf];
        let tpPrices = [];
        if (cy.entry && s && s.entry && Math.abs(s.entry - cy.entry) < 0.5) {
          tpPrices = [s.tp1, s.tp2, s.tp3, s.tp4, s.tp5].filter(Boolean);
        }
        const entryAlert = d.entries.find(function(a) { return (a.cycle || 0) === cNum; });
        if (entryAlert && entryAlert.tp) {
        ...
        else { status = '🟢 Active ' + cy.tps.size + '/5 TPs'; statusClass = 'green'; }
